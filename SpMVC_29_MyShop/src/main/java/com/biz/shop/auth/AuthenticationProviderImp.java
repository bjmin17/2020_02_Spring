...
/*
 * @Controller, @Service 등과 같이 자동으로 scan 되어 bean으로
 * 등록할 클래스를 지정
 * Controller, Service, Repository 등과 같이 기능을 명시적으로 지정하기 애매한 클래스는 Component로 지정을 한다.
 * 
 * spring security에서 login을 직접 수행하는 클래스
 * login을 customizing 하여
 * UserDetailsService로부터 DB를 조회하여
 * 사용자 정보를 받아오고
...
public class AuthenticationProviderImp implements AuthenticationProvider {
	...
	// @RequiredArgsConstructor를 선언하여 사용할 수 있도록 초기화를 수행한다.
